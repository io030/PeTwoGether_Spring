...
<section class="ftco-section bg-light">
	...
	<div class="formContainer"
		...
		<form method="POST" action="/petwogether/member/psreviewaddok" enctype="multipart/form-data">

			<div class="mb-3">
				<label for="exampleFormControlInput1" class="form-label" style="color: #F8A1A4;">돌봄 서비스</label> 
				<select id="csseq" name="csseq" class="form-select" size="2" aria-label="size 2">
					<c:forEach items="${cList}" var="dto">
						<option value="${dto.csseq}">${dto.csseq},${dto.csstart } ~ ${dto.csend}</option>
					</c:forEach>
				</select>
			</div>
			<div class="mb-3">
				<label for="exampleFormControlInput1" class="form-label" style="color: #F8A1A4;">제목</label> 
				<input type="text" class="form-control" id="psrtitle" name="psrtitle" placeholder="제목을 입력하세요" required style="height: 1000px;">
			</div>
			<div class="mb-3">
				<label for="exampleFormControlInput1" class="form-label" style="color: #F8A1A4;">내용</label>
				<textarea class="form-control" rows="3" style="resize: none; height: 800px !important;" name="psrcontent" id="psrcontent" required placeholder="내용을 입력하세요"></textarea>
			</div>

			<div style="margin-bottom: 5px;">
				<input id="psrrate" name="psrrate" value="0" type="number" class="rating" data-symbol="❤" min=0 max=5 step=0.5 data-size="sm" style="display: none;">
			</div>
